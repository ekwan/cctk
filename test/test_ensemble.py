...
class TestEnsemble(unittest.TestCase):
    def test_indexing(self):
        ...
        self.assertTrue(isinstance(mols.molecules[0], cctk.Molecule))
        self.assertEqual(len(mols[0:10]), 3)
        self.assertListEqual(mols[0:10,"energy"], [-1159.56782625, -1159.56782622, -1159.56782622])

        result = mols[0:10,"enthalpy"]
        self.assertListEqual(result, [None, None, -1159.314817])
        with self.assertRaises(ValueError):
            mols[0,"adsfadsf"]
        with self.assertRaises(ValueError):
            mols[0:10,"enthalpydfsdf"]

        self.assertListEqual(list(mols.keys()), list(mols._items.keys()))

        molecule = mols.molecules[1]
        mols[molecule,"energy"] = 300
        ...
        mols[[mols.molecules[1],mols.molecules[2]],"energy"] = [100, 101]
