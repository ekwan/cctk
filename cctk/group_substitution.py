...
def add_group_to_molecule(molecule, group, add_to):
    ...
    old_radius = get_covalent_radius(molecule.atoms[add_to-1])
    new_radius = get_covalent_radius(group.atoms[attach_to-1])
    delta_rad = new_radius - old_radius
    
    #### make the swap! (this only adds the atoms, still have to get the geometry right)
    molecule.atoms[add_to-1] = group.atoms[attach_to-1]
    new_indices = [i + len(molecule.atoms) for i in range(len(other_indices))]
    ...
    assert (len(molecule.atoms) == len(molecule.geometry)), f"molecule has {len(molecule.atoms)} atoms but {len(molecule.geometry)} geometry elements!"
